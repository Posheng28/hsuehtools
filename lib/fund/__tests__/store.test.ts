...
const fund: FundSnapshot = {
  fundId: 'test-fund', reportType: 'monthly_top10', period: '2026-04',
...
describe('fundStore', () => {
  ...
  afterEach(async () => {
    await fs.rm(path.join(process.cwd(), 'data/funds/test-fund'), { recursive: true, force: true })
    await fs.rm(path.join(process.cwd(), '.funddata/etf/TEST00A'), { recursive: true, force: true })
  })

  it('基金月報存到 data/funds/（committed 區）', () => {
    expect(snapshotPath(fund)).toContain(path.join('data', 'funds', 'test-fund'))
  })
  ...
  it('存後可讀回', async () => {
    ...
    const got = await loadSnapshot('test-fund', 'monthly_top10', '2026-04')
