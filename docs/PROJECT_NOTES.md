...
**處置計數對應**：第一款計規則①（連3日）；**第一款~第八款**任一計規則②③④（連5日6次/10日6次/30日12次）。**款九~十四不計**——依 FL007225 第6條第1項第2款，「任一款」僅指第一款至第八款；如第十三款＝當日沖銷成交量比率，雖被列注意但**不累積進處置計數**（2026-06 修正：先前誤把非第一款一律當 level 2 計入）。
...
- 歷史注意 level（`lib/disposal/noticeLevel.ts` 的 `classifyNoticeLevel(info)`，`notices/route.ts` 套用）：含「第一款」→ **1**；`/第[二三四五六七八]款/` → **2**；**僅 `/第[九十]/`（款九~十四，如第十三款當沖）→ 0，API 直接 `continue` 整筆排除、不進 `records`**；無可辨識款別 → 保守當 **2**（避免漏算）。在源頭排除最乾淨：`computeTriggers`/`getRules` 都「窗口內全算」，level 0 不入庫即兩路徑自動正確。**規則①只算 level 1**（模擬日的 level 2=款一②則算第一款）。子字串安全：`'第十一款'.includes('第一款')===false`、`/第[二三四五六七八]款/` 不誤中「第十X款」。
